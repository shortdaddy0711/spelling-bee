...
const Words = () => {
  ...
  const handleWords = (str) => {
    let wordsObj = {};
    let randWords = (str === 'missed' ? wrong : Arr[str])
  ...
  const handlePlay = async (e) => {
    ...
    const url = `https://www.dictionaryapi.com/api/v3/references/collegiate/json/${e.currentTarget.value}?key=${apiKey}`;
    fetch(url)
      .then((resp) => {
        return resp.json();
      })
  ...
  return (
    <div className='body'>
      <div className='header'>
        ...
        {words &&
          Object.keys(Arr).map((element) => {
            return (
              <button
                className={
                  level === element ? 'btn-level-selected' : 'btn-level'
                }
                onClick={() => {
                  setLevel(element);
                  scrollToStart();
        ...
        <button
          className={level === 'missed' ? 'btn-level-selected' : 'btn-level'}
          onClick={() => {
            setLevel('missed');
            scrollToStart();
            handleWords('missed');
      ...
      <div className='container' id='container'>
        {words.map((word, i) => (
          <div className={level} key={i}>
            ...
            <div className='word-element'>{showWords[word] ? word : '?'}</div>
            <div className='word-result' id={`${level}-${word}`}></div>
            ...
            <div className='btn-control'>
              <button className='btn-play' onClick={handlePlay} value={word}>
                <i className='fas fa-play'></i>
              </button>
              <button className='btn-recog' onClick={handleSpeech} value={word}>
                <i className='fas fa-microphone-alt'></i>
              </button>
            </div>
            <div className='btn-nav'>
              <button className='btn-nav-left' onClick={scrollToLeft}>
              ...
              <button className='btn-nav-right' onClick={scrollToRight}>
      ...
      <div className='instruction'>
        ...
        <ol>
          ...
          <li>
            if you want to see the word before answer, click the 'Show word'
            button
          </li>
        </ol>
      </div>
